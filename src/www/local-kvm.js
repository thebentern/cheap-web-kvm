...
async function startStream() {
    try {
        ...
        const supportedVidPidPairs = [
            { vid: '534d', pid: '2130' , product: "cheap-web-kvm (MS2130)"}, // MS2130, preferred: 1080p60
            { vid: '345f', pid: '2130' , product: "cheap-web-kvm (MS2130 alt)"}, // rebadged MS2130 dongles
        ...
        if (!videoDevice) {
            console.error('HDMI capture device not found');
            $('body').toast({
                message: '<i class="red exclamation triangle icon"></i> HDMI capture device (MS2130 / MS2109) not found. Please connect the device and try again.'
            });
        ...
        if (!audioDevice) {
            console.warn('HDMI capture audio device not found');
        }
